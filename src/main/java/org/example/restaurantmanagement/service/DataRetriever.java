...
public class DataRetriever {
    ...
    public Order saveOrder(Order order) {
        ...
        try (Connection conn = new DBConnection().getDBConnection()) {
            ...
            List<DishOrder> dishOrderList = order.getDishOrders();
        ...
        try (Connection connection = dbConnection.getDBConnection();){
            ...
            try {
                ...
                while (resultSet.next()) {
                    ...
                    stockValue.setUnit(UnitTypeEnum.valueOf(resultSet.getString("unit")));
    ...
    private void attachIngredients(Connection conn, List<DishIngredient> ingredients)
        ...
        try (PreparedStatement ps = conn.prepareStatement(attachSql)) {
            for (DishIngredient dishIngredient : ingredients) {
                ...
                ps.setObject(5, dishIngredient.getUnitType());
                ps.addBatch(); // Can be substitute ps.executeUpdate() but bad performance
            }
            ps.executeBatch();
        }
        ...
        try (Connection connection = dbConnection.getDBConnection();){
            ...
            try {
                ...
                while (resultSet.next()) {
                    ...
                    dishIngredient.setUnitType(UnitTypeEnum.valueOf(resultSet.getString("unit")));
